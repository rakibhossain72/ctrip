...
export default function PaymentDetailView({
  ...
  const progressPercent = isTerminated
    ? 0
    : Math.min(100, (timeLeft / 1800) * 100);
  ...
  const handleSweep = async () => {
    if (!payment) return;
    try {
      const result = await sweepAddress.mutateAsync({
        address: payment.address,
        chain_name: sweepChain,
      });
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-xs text-brand-muted">
        <button
          onClick={onBack}
          className="inline-flex items-center gap-1 hover:text-brand-text transition-colors cursor-pointer font-semibold"
        >
          <ArrowLeft className="h-3 w-3" /> Back to Payments
        </button>
        <span className="text-brand-dim">/</span>
        <span className="font-mono text-brand-dim select-all">
          {payment.id}
        </span>
      </div>
      ...
      {!isTerminated && payment.status === "pending" && (
        <div className="rounded-xl border border-amber-200 bg-amber-50/30 p-4 flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2 text-xs text-amber-800">
            ...
            <span>
              Awaiting payment detection. Session expires in{" "}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="rounded-xl border border-brand-border bg-brand-surface shadow-sm overflow-hidden">
            ...
            <div className="px-5 py-2 divide-y divide-brand-border text-xs font-sans">
              <div className="flex justify-between items-center py-3 gap-4">
                ...
                <span className="inline-flex items-center rounded bg-zinc-100 px-2 py-0.5 text-[10px] font-bold text-zinc-800 tracking-wide uppercase">
                  {payment.chain}
                </span>
              </div>
              {/* created api */}
              ...
              <div className="flex justify-between items-center py-3 gap-4">
                ...
                <div className="flex items-center gap-2 justify-end">
                  <span
                    className="font-mono text-brand-muted text-xs truncate max-w-[160px] sm:max-w-xs"
                  ...
                  <button
                    type="button"
                    onClick={copyAddress}
                    className="inline-flex items-center gap-1 rounded border border-brand-border-dark bg-white px-2 py-1 text-[10px] font-semibold text-brand-text hover:bg-brand-bg transition-colors cursor-pointer"
                  >
                    {copied ? (
              ...
              <div className="flex justify-between items-center py-3 gap-4">
                ...
                <span className="font-mono text-brand-text font-semibold">
                  {parseInt(payment.amount_wei || "0").toLocaleString()}
                </span>
              </div>
              <div className="flex justify-between items-center py-3 gap-4">
                <span className="text-brand-muted font-medium">
            ...
            <div className="px-5 py-2 divide-y divide-brand-border">
              {payment.transactions?.length ? (
                payment.transactions.map((tx) => (
                  <div
                    className="flex justify-between items-center py-3 text-xs gap-4"
                    key={tx.id}
                  >
                    <div className="flex flex-col gap-1 min-w-0">
                      <span
                        className="font-mono text-brand-text font-medium select-all break-all pr-4 truncate"
                        title={tx.tx_hash}
                      >
                        {tx.tx_hash}
                      </span>
                      <div className="text-[10px] text-brand-muted">
                        Block {tx.block_number ?? "?"} · {tx.confirmations}{" "}
                        confirmations
                      </div>
                    </div>
                  ...
                  {payment.webhooks.map((wh, index) => (
                    <div className="relative flex gap-4" key={wh.id}>
                      <div className="flex flex-col items-center shrink-0">
                        <div
                          className={`h-2.5 w-2.5 rounded-full border-2 ${wh.status === "success" ? "bg-emerald-500 border-emerald-200" : wh.status === "failed" ? "bg-red-500 border-red-200" : "bg-amber-500 border-amber-200 animate-pulse"}`}
                        />
                        {index < (payment.webhooks?.length ?? 0) - 1 && (
                          <div className="w-px flex-1 bg-brand-border my-1.5" />
                        )}
                      </div>
                      <div className="flex-1 pb-2 text-xs min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2 text-[11px] text-brand-muted">
                          <span
                            className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-bold border ${wh.status === "success" ? "bg-emerald-50 text-emerald-700 border-emerald-100" : wh.status === "failed" ? "bg-red-50 text-red-700 border-red-100" : "bg-zinc-100 text-zinc-500 border-zinc-200"}`}
                          >
                            {wh.status}
                          </span>
                          <span>·</span>
                          <span>
                            {wh.retry_count} retr
                            {wh.retry_count === 1 ? "y" : "ies"}
                          </span>
                          <span>·</span>
                          <span
                            className="font-mono select-all truncate max-w-[140px] sm:max-w-xs"
                        ...
                        {wh.last_error && (
                          <div className="mt-2 text-[11px] text-red-600 font-medium bg-red-50/50 border border-red-100 rounded-md p-2 break-all">
                            ✕ {wh.last_error}
                          </div>
                        )}
                        {wh.next_retry_at && (
                          <div className="mt-1 text-[10px] text-brand-dim">
                            Next retry: {formatDate(wh.next_retry_at)}
                          </div>
                        )}
                      </div>
                    </div>
        ...
        <div className="space-y-6">
          <div className="rounded-xl border border-brand-border bg-brand-surface shadow-sm overflow-hidden p-5 space-y-4">
            <h3 className="text-sm font-semibold text-brand-text flex items-center gap-2 pb-3 border-b border-brand-border">
              Operations Control
            </h3>
          ...
          <div className="rounded-xl border border-brand-border bg-brand-surface shadow-sm overflow-hidden p-5 space-y-4">
            ...
            <div className="space-y-3">
              <div className="space-y-1.5">
                <label className="text-[11px] font-semibold text-brand-muted">
                  Select Event Type
                </label>
                <select
              ...
              {webhookResult && (
                <div className="text-[11px] bg-brand-bg border border-brand-border rounded-lg p-2.5 font-mono text-brand-muted break-all">
                  {webhookResult}
                </div>
              )}
            </div>

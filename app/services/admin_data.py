"""
Shared data-access functions for the admin console.

Single source of truth for the admin queries and mutations, consumed by both
the JSON API routers (``app.api.admin``, ``app.api.analytics``) and the
server-rendered HTML pages (``app.api.admin_ui``).
"""
...
async def transaction_stats(db: AsyncSession) -> TransactionStats:
...
async def payment_volume(db: AsyncSession) -> PaymentVolumeSummary:
    """Total payment counts and volume broken down by status."""
    return _volume_summary(await _payment_volume_rows(db))


async def daily_volume(db: AsyncSession, days: int = 30) -> List[DailyVolume]:
...
async def recent_payments(
    ...
    """Most recent payments, optionally filtered by status / chain / free-text."""
    q = select(Payment).order_by(Payment.created_at.desc()).limit(limit)
    if status:
        try:
            q = q.where(Payment.status == PaymentStatus(status.lower()))
        except ValueError:
            pass
    if chain:
        q = q.join(Chain, Payment.chain_id == Chain.id).where(Chain.name == chain.lower())
    if search:
        pattern = f"%{search}%"
        q = q.where(
            Payment.address.like(pattern)
            | Payment.id.cast(String).like(pattern)
            | Payment.amount_raw.cast(String).like(pattern)
        )
...
async def payment_detail(db: AsyncSession, payment_id) -> PaymentDetail:
...
async def list_api_keys(db: AsyncSession, limit: int = 100, offset: int = 0) -> List[ApiKey]:
    """API keys (active and revoked) ordered newest first."""
    result = await db.execute(
...
async def create_api_key(db: AsyncSession, user: User, name: str) -> Tuple[ApiKey, str]:
    """Create an API key; returns (key record, raw key shown only once)."""
...
async def revoke_api_key(db: AsyncSession, key_id) -> ApiKey:

...
class WorkerClient:
    ...
    async def trigger_payment_scan(self) -> str:
        ...
        logger.info("Enqueued payment scan job: %s", job.job_id)
        return job.job_id
    ...
    async def process_payment(self, payment_id: int, chain_name: str) -> str:
        ...
        logger.info("Enqueued payment processing job: %s", job.job_id)
        return job.job_id
    ...
    async def trigger_sweep(self) -> str:
        ...
        logger.info("Enqueued sweep job: %s", job.job_id)
        return job.job_id
    ...
    async def sweep_address(self, address: str, chain_name: str) -> str:
        ...
        logger.info("Enqueued address sweep job: %s", job.job_id)
        return job.job_id
    ...
    async def send_webhook(self, payment_id: int, event_type: str) -> str:
        ...
        logger.info("Enqueued webhook job: %s", job.job_id)
        return job.job_id
    ...
    async def send_custom_webhook(
        ...
        logger.info("Enqueued custom webhook job: %s", job.job_id)
        return job.job_id
